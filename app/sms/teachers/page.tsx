...
export default function SMSTeachersPage() {
  const { teachers, timetable, classes, addTeacher, updateTeacher, deleteTeacher } = useSMS();
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full bg-[#0b121e] border border-[#1e293b] pl-9 pr-3 py-2.5 rounded-xl text-xs text-white placeholder-gray-500 focus:outline-none focus:border-emerald-500"
        />
      </div>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-bold text-emerald-400 font-mono text-[10px]">{t.employeeCode}</span>
                    <div className="flex gap-1">
                      <button
                        ...
                        className="p-1 hover:bg-emerald-500/20 text-gray-400 hover:text-emerald-300 rounded"
                        title="Edit Teacher"
                      >
                        <Edit2 size={12} />
                      </button>
                  ...
                  <div>
                    <h4 className="font-black text-white text-sm">{t.fullName}</h4>
                    <div className="text-xs text-gray-400">{t.designation} • <span className="text-emerald-300 font-semibold">{t.department}</span></div>
                  </div>

                  <div className="pt-2 border-t border-gray-800 flex justify-between items-center text-[10px]">
                    <span className="text-gray-500">Phone: {t.phone}</span>
        ...
        <div className="lg:col-span-2 space-y-6">
          {selectedTeacher ? (
            <div className="space-y-6">
              {/* Teacher Header Card */}
              <div className="bg-[#0b121e] border border-emerald-500/30 rounded-2xl p-6 space-y-4">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h2 className="text-lg font-black text-white">{selectedTeacher.fullName}</h2>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-emerald-600/20 hover:bg-emerald-600 text-emerald-300 hover:text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5"
                    >
                      <Edit2 size={13} />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 text-xs border-t border-gray-800">
                  <div className="bg-black/40 p-3 rounded-xl">
                    <span className="text-[10px] uppercase font-bold text-gray-400 block">Department</span>
                    <b className="text-white">{selectedTeacher.department}</b>
                  </div>
                  <div className="bg-black/40 p-3 rounded-xl">
                    <span className="text-[10px] uppercase font-bold text-gray-400 block">Assigned Subjects</span>
                    <b className="text-emerald-400">{(selectedTeacher.assignedSubjects || ["General"]).join(", ")}</b>
                  </div>
                  <div className="bg-black/40 p-3 rounded-xl">
                    <span className="text-[10px] uppercase font-bold text-gray-400 block">Contact Phone</span>
                    <b className="text-sky-300">{selectedTeacher.phone}</b>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0b121e] border border-[#1e293b] rounded-2xl overflow-hidden shadow-xl">
                <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-black/40">
                  ...
                  <span className="text-[10px] text-emerald-400 font-mono">Live Timetable Sync</span>
                </div>
                      ...
                      {teacherPeriods.length === 0 ? (
                        ...
                        teacherPeriods.map((p) => (
                          <tr key={p.id} className="hover:bg-white/[0.02] transition">
                            <td className="p-4 font-bold text-emerald-400">Period #{p.periodNumber}</td>
                            <td className="p-4 font-bold text-white">{p.timeSlot}</td>
                            <td className="p-4 font-sans font-bold text-sky-300">
                              {p.className} ({p.sectionName})
                            </td>
                            <td className="p-4 font-sans font-bold text-white">{p.subject}</td>
                            <td className="p-4 text-gray-400 font-sans">{p.room}</td>
            ...
            <div className="p-12 text-center text-gray-500 bg-[#0b121e] border border-[#1e293b] rounded-2xl">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-[#0b121e] border border-emerald-500/40 rounded-3xl w-full max-w-lg shadow-2xl p-6 my-8 animate-fade-in-up">
            ...
            <form onSubmit={handleSave} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] uppercase font-bold text-gray-400 mb-1">Qualification *</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] uppercase font-bold text-emerald-400 mb-1">Phone Number *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-gray-400 mb-1">Monthly Salary (PKR)</label>
                  <input
                    type="number"
                    value={form.salary}
              ...
              <button
                type="submit"
                className="w-full py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black uppercase rounded-xl transition text-xs shadow-lg"
              >
